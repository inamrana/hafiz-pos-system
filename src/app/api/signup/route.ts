...
export async function POST(req: NextRequest) {
  ...
  const shop = await shopsRepo.create({ name: shopName.trim(), ownerName: ownerName.trim(), ownerEmail: ownerEmail.trim() });

  try {
    const user = await withTenant(shop.id, async () => {
      await settingsRepo.update({ shopName: shopName.trim() });
