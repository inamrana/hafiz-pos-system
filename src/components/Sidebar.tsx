...
const mainItems = [
  ...
  { href: '/billing', label: 'Billing', icon: ShoppingCart, adminOnly: false },
  { href: '/stock', label: 'Stock', icon: Package, adminOnly: false },
  { href: '/stock?filter=low', label: 'Low Stock', icon: AlertTriangle, adminOnly: false },
  { href: '/purchases', label: 'Purchases', icon: Truck, adminOnly: false },
  { href: '/suppliers', label: 'Suppliers', icon: Building2, adminOnly: false },
  { href: '/udhaar', label: 'Udhaar Khaata', icon: BookOpen, adminOnly: false },
  { href: '/reports', label: 'Bills', icon: Receipt, adminOnly: false },
];

const reportItems = [
  { href: '/reports/daily', label: 'Daily Report', icon: CalendarDays, adminOnly: false },
  { href: '/reports/monthly', label: 'Monthly Report', icon: CalendarRange, adminOnly: false },
  { href: '/reports/yearly', label: 'Yearly Report', icon: TrendingUp, adminOnly: false },
  { href: '/reports/inventory', label: 'Inventory Report', icon: Boxes, adminOnly: false },
];

const bottomItems = [
  { href: '/users', label: 'Cashiers', icon: Users, adminOnly: true },
  { href: '/settings', label: 'Settings', icon: SettingsIcon, adminOnly: true },
];

const STORAGE_KEY = 'hp_sidebar_expanded';

export default function Sidebar({ shopName, role }: { shopName: string; role: 'ADMIN' | 'CASHIER' }) {
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 p-3 space-y-1 overflow-y-auto overflow-x-hidden">
        ...
        {bottomItems.filter((n) => role === 'ADMIN').map(renderLink)}
      </nav>

      {/* Footer */}
      <div className="p-4 border-t border-slate-700">
        <p className="text-xs text-slate-500 text-center">{expanded ? 'Mart POS v2.0' : 'v2.0'}</p>
      </div>
    </aside>
  );
}
